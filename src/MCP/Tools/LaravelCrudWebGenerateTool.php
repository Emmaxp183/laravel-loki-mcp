...
#[Description('Generate conventional Laravel web CRUD controllers and views.')]
class LaravelCrudWebGenerateTool extends Tool
{
    use AuditsToolCalls;

    protected string $name = 'laravel-crud-web-generate';
    ...
    public function handle(Request $request, CrudGenerator $generator, EnvironmentPolicy $environmentPolicy): ResponseFactory
    {
        ...
        if (! $environmentPolicy->codeEditsEnabled($environment)) {
            return $this->auditedResponse($this->name(), $request, [
                'summary' => 'CRUD web generation request was denied.',
                'data' => [
                    'allowed' => false,
        ...
        $result = $generator->generate('web', $validated['resource'], $validated['fields']);

        return $this->auditedResponse($this->name(), $request, [
            'summary' => 'CRUD web flow generated.',
            'data' => $result,
            'warnings' => [],
            'meta' => [
                'module' => 'generators',
